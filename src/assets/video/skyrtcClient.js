export var SkyRTC = function() {
  ...
  var iceServer = {
    'iceServers': [{
      ...
      { 'urls': 'stun:49.233.221.184' },
      { 'urls': 'stun:stunserver.org' },
      { 'urls': 'turn:numb.viagenie.ca', credential: 'muazkh', username: 'webrtc@live.com' },
      { 'urls': 'turn:49.233.221.184', credential: 'ling1234', username: 'ling' }],
  ...
  skyrtc.prototype.connect = function(server, room) {
    ...
    socket.onclose = function(data) {
      ...
      that.localMediaStream.getTracks()[1].stop();
      var pcs = that.peerConnections;
      for (let i = pcs.length; i--;) {
    ...
    this.on('_peers', function(data) {
      ...
      that.me = data.you;
      that.emit('get_peers', that.connections);//实测没有用
